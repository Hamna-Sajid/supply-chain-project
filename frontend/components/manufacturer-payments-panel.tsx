...
export function ManufacturerPaymentsPanel() {
  ...
  const fetchPayments = async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/manufacturer/payments`, {
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/manufacturer/payments/${paymentId}/status`,
        {
          method: "PUT",
          headers: {
      ...
      if (response.ok) {
        ...
        alert(`Failed to update payment status: ${response.status} - ${errorText}`)
      }
    } catch (err) {
      console.error("Error updating status:", err)
      alert(`Error updating payment status: ${err instanceof Error ? err.message : 'Unknown error'}`)
    }
  }
            ...
            {selectedPayment ? (
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="font-medium">{selectedPayment.supplier}</p>
                </div>
